...
setwd("C:/Users/Герман/Downloads/Files for the class-20240711") #(you change it to the path to your folder)
...
str(pi_data)
wilcox.test(Am ~ Group, data=pi_data)
wilcox.test(AQY ~ Group, data=pi_data)

# or, using the long table format, do it for the whole bunch         

res <- pi_data_long %>% group_by(variable) %>% 
  do(w = wilcox.test(Value~Group, data=.)) %>% 
...
ggplot(Photosurvey, aes(x=Year, y=Algae ))+
...
filtered_data <- Photosurvey %>% 
  filter(Echinodermata != 0)
...
bartlett.test(Photosurvey$Mollusca~Photosurvey$site)

#2) test difference in coverage levels

kruskal.test(Mollusca~site, data=Photosurvey)
...
ggplot(Photosurvey, aes(x=Year, y=Algae, fill=site))+
  geom_boxplot(outlier.shape = 9)+
  geom_point(position=position_jitterdodge(jitter.width = 0.4), size=1)+
  theme_light()+
  facet_grid(.~depth)

ggplot(Photosurvey, aes(x = Algae, y = live_coverage, color = site)) +
  geom_point() +
  labs(
    title = "Relationship between Algae and Bryozoa Coverage",
    x = "Algae Coverage (%)",
...
ggplot(Photosurvey, aes(x=Algae, y=live_coverage))+
  geom_boxplot(outlier.shape = NA)+
  geom_point(aes(color=as.factor(depth)),position=position_jitter(width=0.2))+
  theme_light()
...
bartlett.test(Photosurvey$Algae~Photosurvey$live_coverage)

#2) test difference in coverage levels

kruskal.test(Algae~Bryozoa, data=Photosurvey)
...
ggplot(Photosurvey, aes(x=Soft.Substrate, y=Algae))+
  geom_point()+
